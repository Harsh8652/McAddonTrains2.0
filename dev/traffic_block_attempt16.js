// Stage10 ATTEMPT16 layered on REAL Stage9 ATTEMPT5 c8cb356b - traffic fixes
// ATTEMPT16 (fix-forward on ATTEMPT15 359d7fb7, which is byte-verified Stage9 ATTEMPT5 c8cb356b + one additive hunk):
//  1. release-behind + destination/completion releases wired into the coordinator; EVERY release path uses
//     safelyReleaseTrafficReservation (occupied => NEVER release; success => drop tracking; authoritative
//     foreign-owner/free => drop tracking; failed/unknown => RETAIN tracking for retry; occupancy never mutated).
//  2. arbitration uses new read-only authoritative APIs reservationManager.getOwner / occupancyManager.getOccupant
...
//  5. fixed ATTEMPT15 latent arg-order bug: resolveLeadingCurrentSection is (trainId, world, route, occupiedSet).
...
// Single safe release helper. occupiedSet = sections occupied by THIS train (authoritative Stage5 view).
// MUST NOT mutate occupancyManager.knownTrainToSections or any occupancy state.
function safelyReleaseTrafficReservation(trainId, sectionIds, world, rec, occupiedSet){
 try{
  ...
  for(const secId of sectionIds){
   try{
    if(!secId) continue;
    if(occupiedSet && occupiedSet.has && occupiedSet.has(secId)) continue; // occupied => NEVER release
...
if(typeof system9!=='undefined'){
 ...
 try{const w=__realWorldForTraffic; if(w&&w.getDynamicProperty){loadTrafficOnWorldLoad(w);}}catch(e){}
}
}catch(e){console.warn('[Stage10 ATTEMPT16] traffic wrapper',e);}
