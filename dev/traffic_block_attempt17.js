// Stage10 ATTEMPT17 layered on REAL Stage9 ATTEMPT5 c8cb356b - traffic fixes
// ATTEMPT17 (fix-forward on ATTEMPT16 0fb89ec4; audit-rejected items fixed, architecture unchanged):
//  1. OCCUPANCY SAFETY CONTRACT for releases (helper, below):
//     known occupied by this train => NEVER release; unknown/unloaded occupancy => NEVER release;
//     occupancy probe throws/fails => NEVER release; a release call happens ONLY when occupancy is
//     POSITIVELY known NOT occupied (authoritative KNOWN_EMPTY record in the section's own occupancy
//     shard). "Absent from loaded shards" is NEVER treated as "free".
//  2. PERFORMANCE: occupancy lookups resolve the section's authoritative occupancy shard DIRECTLY via
//     occupancyManager._resolveOccupancyRecord (Stage8 SectionLocationIndex / Section.rootPos) - no
//     iteration over loaded occupancy shards, no world scans.
// ATTEMPT16 (carried forward unchanged):
//  1. release-behind + destination/completion releases wired into the coordinator; EVERY release path uses
//     safelyReleaseTrafficReservation (owner success / authoritative foreign-owner / authoritative free =>
//     drop tracking; failed/unknown => RETAIN tracking for retry; occupancy never mutated).
//  2. arbitration uses read-only authoritative APIs reservationManager.getOwner / occupancyManager.getOccupant
//     (added in place to the existing Stage7/Stage5 managers; no duplicated ownership state).
//  3. persistence: TRAFFIC_VERSION-versioned records, deterministic rotating save cursor (advances after each
//     successful save), DP removed when a TrafficRecord is definitively deleted, loadAll wired at world load.
//  4. priority arbitration is pure in (effectivePriority, trainId): independent of rotating cursor, Map insertion
//     order, and TrafficRecord pre-existence; each batch is sorted before processing.
//  5. fixed ATTEMPT15 latent arg-order bugs: resolveLeadingCurrentSection(trainId, world, route, occupiedSet)
//     and reservationManager.reserveSections(sectionIds, trainId, world).
//  6. per-train claimedSections index; stale claims of non-autonomous trains cleaned via own index (no global scans).
//  7. reserve+release calls counted together against TRAFFIC_MAX_MUTATIONS_PER_TICK; horizon still 5 sections.
//  8. removed ATTEMPT15 dead scaffolding (unused no-op priority index, unused shard map, hash%1000 pseudo-priority).
...
// ATTEMPT17 occupancy safety contract (per candidate section):
//   occupiedSet hit (known self-occupied) => NEVER release.
//   authoritative foreign owner / authoritative already-free => drop tracking (tracking reconciliation only;
//     no world mutation, no release call).
//   releaseSection is called ONLY when occupancy is POSITIVELY known NOT occupied
//     (authoritative KNOWN_EMPTY via occupancyManager.getOccupancyState). Unknown/unloaded/probe-failed/
//     KNOWN_OCCUPIED/CONFLICT/UNKNOWN occupancy => NEVER release, RETAIN tracking for retry.
// MUST NOT mutate occupancyManager state or occupancyManager.knownTrainToSections.
 ...
 try{
  ...
  for(const secId of sectionIds){
   try{
    ...
    let owner=null; let ownerKnown=false;
    try{ if(typeof reservationManager!=="undefined" && typeof reservationManager.getOwner==="function"){ owner=reservationManager.getOwner(secId); ownerKnown=true; } }catch(e){ ownerKnown=false; }
    if(ownerKnown && owner && owner!==trainId){ toRemove.push(secId); continue; } // authoritative foreign owner => our tracking stale
    if(ownerKnown && owner===null){ toRemove.push(secId); continue; } // authoritative: already free
    // occupancy must be POSITIVELY known NOT occupied, else NEVER release (unknown/unloaded/failed != free)
    let occState=null;
    try{ if(typeof occupancyManager!=="undefined" && typeof occupancyManager.getOccupancyState==="function"){ occState=occupancyManager.getOccupancyState(secId, world); } }catch(e){ occState=null; }
    ...
    // failed or unknown ownership => RETAIN tracking for retry (never forget)
   }catch(e){}
  }
...
}catch(e){console.warn('[Stage10 ATTEMPT17] traffic wrapper',e);}
